"""
Vision-based structured extraction using the Gemini API (free tier).

Why a VLM/cloud model instead of PaddleOCR/TrOCR or local inference:
Your source sheets are handwritten, multi-script (Gujarati + Hindi + English),
multi-writer, and tabular with inconsistent column layouts. Character-recognition
OCR engines (PaddleOCR, TrOCR) segment glyphs independently and have no way to use
surrounding context to resolve ambiguous strokes. A VLM reads the whole image
holistically and can be instructed to directly emit the table structure you want.

Local inference was dropped in favor of Gemini's free API tier because the
development machine has no dedicated GPU (Intel UHD integrated graphics only),
which makes local VLM inference on CPU impractically slow (5+ min per image,
often timing out). Gemini Flash models are fast, free (no credit card required),
and strong at OCR/handwriting.
"""
...
# Downscaling keeps handwriting legible while cutting token cost/latency. Gemini
# handles large images fine, so this is mostly about staying comfortably inside
# free-tier token budgets, not a hard requirement like it was for local inference.
...
def _call_gemini_vision(image_path: str) -> str:
    ...
    try:
        ...
        # Common cause: response was blocked by safety filters, or candidates is empty
...
def _extract_json_block(text: str) -> dict:
    """responseMimeType='application/json' should give clean JSON, but this stays as a
    safety net in case of stray markdown fences or leading/trailing text."""
...
def extract_structured_data(image_path: str) -> dict:
    ...
    Main entry point. Returns:
    {
        "location_name": str,
        "report_date": str,
        "rows": [ {sr, bus_no, mech, work_done, material}, ... ],
        "raw_output": str,
    }
